...
"ctx.delete": "Delete",
		...
		function ReadingView({ text, t, fontFamily, fontSize, lineSpacing, bg, margin, state, onState, flipMode, searchQuery, searchActive, searchMatchesRef, path }) {
			...
			react.useEffect(() => {
				...
				if (!(typeof state.paragraph === "number" && Number.isFinite(state.paragraph))) return;
				if (restoredRef.current) return;
				if (flipMode === "scroll") {
					const el = bodyRef.current;
					if (el !== null) {
						const target = el.querySelector(`[data-para-index="${state.paragraph}"]`);
						if (target !== null) {
				...
				const idx = pages.findIndex((page) => page.some((s) => s.paraIndex === state.paragraph));
				if (idx >= 0) {
					setPageIndex(idx);
					restoredRef.current = true;
				}
			...
			const isBookmarkedHere = (() => {
				...
				return (state?.bookmarks ?? []).some((b) => b.paragraph === paragraph);
			})();
			const addBookmark = () => {
				const paragraph = currentParagraph();
				if (paragraph === null) return;
				const existing = state?.bookmarks ?? [];
				if (existing.some((b) => b.paragraph === paragraph)) return;
			...
			const removeBookmarkHere = () => {
				const paragraph = currentParagraph();
				if (paragraph === null) return;
				const list = (state?.bookmarks ?? []).filter((b) => b.paragraph !== paragraph);
				onState({ bookmarks: list });
				persistProgress(state?.progress ?? null, list);
			};
				...
				children: [
					react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							return react_jsx_runtime.jsxs("div", {
								children: [
									chapter !== void 0 && react_jsx_runtime.jsx("div", { className: css.readingHeading, children: chapter.raw }),
												...
												children: [
													...
													react_jsx_runtime.jsx("button", {
														...
														onClick: (event) => {
															...
															onState({ bookmarks: (state?.bookmarks ?? []).filter((x) => x.id !== b.id) });
														},
						...
						children: [
							...
							(writingMode || readingMode) && react_jsx_runtime.jsxs("div", {
								...
								children: [
									...
									searchOpen && react_jsx_runtime.jsx("div", {
