window.__ModuleLoader__.load({
	...
	let react_dom = require("react-dom");
		...
		const css = {
			...
			"shelfToggleActive": "rh_shelfToggleActive",
			"headerBtn": "rh_headerBtn",
			"headerBtnPrimary": "rh_headerBtnPrimary",
			"ctxMenu": "rh_ctxMenu",
			"ctxItem": "rh_ctxItem",
			"ctxSep": "rh_ctxSep",
			"panel": "rh_panel",
			...
			"sideTabs": "rh_sideTabs",
			"sideTabBtn": "rh_sideTabBtn",
			"sideTabBtnActive": "rh_sideTabBtnActive",
			"shelfRoot": "rh_shelfRoot",
			"shelfSearch": "rh_shelfSearch",
			"shelfSearchInput": "rh_shelfSearchInput",
			"shelfList": "rh_shelfList",
			"shelfRow": "rh_shelfRow",
			"shelfRowActive": "rh_shelfRowActive",
			"shelfRowTitle": "rh_shelfRowTitle",
			"shelfRowMeta": "rh_shelfRowMeta",
			"shelfRowActions": "rh_shelfRowActions",
			"shelfPinBtn": "rh_shelfPinBtn",
			"shelfPinBtnActive": "rh_shelfPinBtnActive",
			"shelfDelBtn": "rh_shelfDelBtn",
			"shelfEmpty": "rh_shelfEmpty",
			"shelfHeader": "rh_shelfHeader"
		};
...
"ctx.delete": "删除",
			...
			"shelf.title": "书架",
			"shelf.entry": "书架",
			"shelf.empty": "书架是空的。用阅读模式读过文本后，它会自动上架。",
			"shelf.searchHint": "搜索书名…",
			"shelf.noMatch": "没有匹配的书",
			"shelf.lastRead": "最近阅读",
			"shelf.pin": "置顶",
			"shelf.unpin": "取消置顶",
			"shelf.pinned": "已置顶",
			"shelf.read": "打开",
			"shelf.remove": "移出书架",
			"shelf.removeConfirm": "要把《{name}》从书架移除吗？",
			"shelf.removeKeepState": "仅移出书架（保留进度书签）",
			"shelf.removeAll": "全部移除（保留文件，删除进度书签）",
			"shelf.removeAllConfirm": "将删除《{name}》的阅读进度与书签（文件本身不受影响），确定吗？",
			"shelf.removed": "已从书架移除",
			"shelf.broken": "《{name}》无法打开（文件可能已移动或删除）。",
			"shelf.brokenRemove": "文件无法打开，是否仍从书架移除？",
			"shelf.openFailed": "打开失败",
			"shelf.readingHint": "已打开阅读模式，定位到上次进度"
		};
		const en = {
...
"ctx.delete": "Delete",
			...
			"shelf.entry": "Bookshelf",
			"shelf.empty": "The shelf is empty. Text read in reading mode appears here automatically.",
			"shelf.searchHint": "Search title…",
			"shelf.noMatch": "No matching books",
			"shelf.lastRead": "Last read",
			"shelf.pin": "Pin",
			"shelf.unpin": "Unpin",
			"shelf.pinned": "Pinned",
			"shelf.read": "Open",
			"shelf.remove": "Remove from shelf",
			"shelf.removeConfirm": "Remove “{name}” from the shelf?",
			"shelf.removeKeepState": "Remove from shelf only (keep progress)",
			"shelf.removeAll": "Remove everything (keep file, drop progress)",
			"shelf.removeAllConfirm": "This deletes the reading progress and bookmarks of “{name}” (the file itself is untouched). Continue?",
			"shelf.removed": "Removed from shelf",
			"shelf.broken": "“{name}” cannot be opened (the file may have moved or been deleted).",
			"shelf.brokenRemove": "The file cannot be opened. Remove it from the shelf anyway?",
			"shelf.openFailed": "Open failed",
			"shelf.readingHint": "Reading mode opened at your last progress"
		};
		...
		const apiShelfGet = () => apiCall("/readwrite-hub-api/shelf");
		const apiShelfRemove = (path, keepState) => apiPost("/readwrite-hub-api/shelf-remove", { path, keepState });
		const apiShelfPin = (path, pinned) => apiPost("/readwrite-hub-api/shelf-pin", { path, pinned });
		...
		function ReadingView({ text, t, fontFamily, fontSize, lineSpacing, bg, margin, state, onState, flipMode, searchQuery, searchActive, searchMatchesRef, path }) {
			...
			react.useEffect(() => {
				...
				if (el === null) return;
					const update = () => {
						...
						// Only push a new layout when a dimension actually changed, so a
						// resize that leaves the content size untouched (common while
						// dragging the tree/panel handle) doesn't trigger a re-render of
						// the whole reading view.
						const cur = layoutRef.current;
						if (next.h === cur.h && next.w === cur.w) return;
			...
			react.useEffect(() => {
				// Measure in BOTH flip modes. Scroll mode renders the full
				// paragraphs (so it does not consume the character slices), but
				// `measured` is still required by the shared progress-restore /
				// page-mapping logic on mode switch — without it the reader can
				// never seek back to a saved paragraph after leaving scroll mode.
				...
				measure.removeChild(probe);
					// Inner content width excludes the body padding. Lines are sliced
					// to full width (no reserved indent) so body lines reach the right
					// edge; the first line of each paragraph is instead rendered with
					// a 2em text-indent, so we cut it 2 characters short to keep the
					// indented first line ending flush with the right edge too.
					const innerW = Math.max(1, contentW);
					...
					for (let i = 0; i < paragraphs.length; i++) {
						...
						// Guard against an empty chapter list: a file with no
						// recognizable heading (e.g. a bare-novel draft of plain
						// paragraphs) yields chapters = [], and chapters[0] would be
						// undefined — reading .paraIndex off it throws a TypeError
						// that crashes ReadingView on mode switch.
						const ch = chIdx < chapters.length && chapters[chIdx].paraIndex === i ? chapters[chIdx] : void 0;
						...
						if (isChapterLine) {
							...
							const slices = [];
							// First line reserves 2 chars for the text-indent.
							slices.push(text.slice(0, firstLineChars));
							for (let off = firstLineChars; off < chars; off += charsPerLine) {
				...
				}, [paragraphs, chapters, fontFamily, fontSize, lineSpacing, contentW, margin, flipMode]);
			...
			const pages = react.useMemo(() => {
				...
				const safety = Math.max(0, Math.round(fontSize * 0.4));
				const next = paginateByLines(measured.paraLines, measured.headHeights, lineH, Math.max(1, viewportH - safety), chapterStarts);
				pagesRef.current = next;
				return next;
			}, [paragraphs, measured, viewportH, chapterStarts, fontSize, lineSpacing, flipMode]);
			...
			const markNaturalFlip = () => {
				...
				// Once the user flips/scrolls on their own, the reading position
				// is theirs and may be saved; until then the restored position is
				// the source of truth and must not be overwritten by the initial
				// page-1 save.
			...
			react.useEffect(() => {
				...
				if (flipMode === "scroll") {
					...
					if (el !== null) {
						...
						if (target !== null) {
							// Use scrollIntoView, not offsetTop: scroll-mode paragraphs
							// carry content-visibility:auto, so off-screen elements are
							// skipped by layout and report offsetTop 0 — computing the
							// scroll position from it clamps to 0 and loses the restore.
			...
			const jumpToParagraph = (paraIndex) => {
				...
				if (flipMode === "scroll") {
					...
					if (target !== null) {
						// scrollIntoView handles content-visibility auto (off-screen
						// offsetTop is 0), unlike el.scrollTop = target.offsetTop - …
						target.scrollIntoView({ block: "start" });
					}
					return;
				}
			...
			const currentParagraph = () => {
				if (flipMode === "scroll") {
					...
					// find the paragraph at the top of the viewport. Paragraphs are
					// rendered in document order == para-index order, so binary-search
					// for the first one whose bottom lands below the viewport top.
					// Walking every one of the ~19k paragraphs and forcing a
					// synchronous layout per element is what froze the reader for
					// minutes when the progress bar was clicked.
					const paras = el.querySelectorAll("p");
					const n = paras.length;
					if (n === 0) return null;
					const viewTop = el.getBoundingClientRect().top + 8;
					let lo = 0;
					let hi = n - 1;
					let ans = -1;
					while (lo <= hi) {
						const mid = (lo + hi) >> 1;
						const bottom = paras[mid].getBoundingClientRect().bottom;
						if (bottom > viewTop) { ans = mid; hi = mid - 1; }
						else { lo = mid + 1; }
					}
					if (ans === -1) ans = n - 1;
					const idx = Number(paras[ans].dataset.paraIndex);
			...
			flushProgressRef.current = () => {
				...
				// Before the saved position is restored the reader is still at
				// page 1; flushing now would overwrite the good saved progress.
				if (!restoredRef.current) return;
				const paragraph = currentParagraph();
				if (paragraph === null) return;
				let chapterIndex = -1;
				...
				children: [
					react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							return react_jsx_runtime.jsxs("div", {
								// Skip layout/paint for off-screen paragraphs so
								// resizing the editor width doesn't reflow every
								// one of the thousands of paragraphs in scroll mode.
								style: { contentVisibility: "auto", containIntrinsicSize: "auto 3em" },
								children: [
									chapter !== void 0 && react_jsx_runtime.jsx("div", { className: css.readingHeading, "data-para-index": String(pi), children: chapter.raw }),
									!isChapterLine && react_jsx_runtime.jsx("p", { className: css.readingPara, "data-para-index": String(pi), style: { margin: "0 0 0.5em", textIndent: "2em" }, children: p })
								]
							}, pi);
						})
						: (() => {
									...
									children: [
										slice.showHeading && chapter !== void 0 && react_jsx_runtime.jsx("div", { className: css.readingHeading, children: chapter.raw }),
											react_jsx_runtime.jsx("p", {
												className: css.readingPara,
												"data-para-index": String(pi),
												"data-slice": String(si),
												style: slice.startLine === 0 ? { textIndent: "2em" } : void 0,
												children: renderSliced(pi, sliced)
											})
									]
								}, `${pi}-${si}`);
								});
							...
							returnChip !== null && react_jsx_runtime.jsx("div", {
								...
								onClick: () => {
									...
									if (flipMode === "scroll") {
										...
										if (el === null) return;
										naturalFlipRef.current = false;
										const tgt = el.querySelector(`[data-para-index="${target}"]`);
										if (tgt !== null) tgt.scrollIntoView({ block: "start" });
										return;
									}
									naturalFlipRef.current = false;
									...
									onClick: (event) => {
										...
										if (flipMode === "scroll") {
											...
											if (el !== null) {
												...
												el.scrollTop = target;
												// Sync the visible fill/pct immediately — the native
												// scroll event fires async and React may batch it
												// away, so the bar otherwise sits at the old pct.
												scrollTopRef.current = target;
												setScrollPct(max > 0 ? Math.round((target / max) * 100) : 0);
											}
										} else {
			...
			// ---- bookshelf --------------------------------------------------
			// Left-pane view listing every shelved book (files with a reading
			// progress bookmark), newest-read first with the pinned ones on top.
			...
			function ShelfView({ books, loading, query, onChangeQuery, onOpen, onPin, onRemove, t, activePath }) {
				const normalized = (typeof query === "string" ? query : "").trim().toLowerCase();
				const filtered = normalized === "" ? books : books.filter((b) => String(b.title ?? "").toLowerCase().includes(normalized) || String(b.path ?? "").toLowerCase().includes(normalized));
				const list = filtered.slice().sort((left, right) => {
					if (left.pinned !== right.pinned) return left.pinned ? -1 : 1;
					return (right.lastReadAt ?? 0) - (left.lastReadAt ?? 0);
				});
				return react_jsx_runtime.jsxs("div", {
					className: css.shelfRoot,
					children: [
						react_jsx_runtime.jsxs("div", {
							className: css.shelfHeader,
							children: [
								react_jsx_runtime.jsx("div", { className: css.shelfSearch, children: react_jsx_runtime.jsx("input", {
									className: css.shelfSearchInput,
									type: "text",
									placeholder: t("shelf.searchHint"),
									value: query,
									onChange: (event) => onChangeQuery(event.target.value),
									"aria-label": t("shelf.searchHint")
								}) })
							]
						}),
						loading ? react_jsx_runtime.jsx("div", { className: css.filesLoading, children: t("files.loading") }) :
						list.length === 0 ? react_jsx_runtime.jsx("div", {
							className: css.shelfEmpty,
							children: normalized === "" ? t("shelf.empty") : t("shelf.noMatch")
						}) : react_jsx_runtime.jsx("div", {
							className: css.shelfList,
							children: list.map((book) => react_jsx_runtime.jsxs("div", {
								className: clsx(css.shelfRow, activePath === book.path && css.shelfRowActive),
								onClick: () => onOpen(book),
								children: [
									react_jsx_runtime.jsxs("div", {
										className: css.shelfRowText,
										children: [
											react_jsx_runtime.jsx("div", { className: css.shelfRowTitle, title: book.title, children: book.title }),
											react_jsx_runtime.jsxs("div", {
												className: css.shelfRowMeta,
												children: [
													!book.exists ? `${t("shelf.broken", { name: book.title })}` : `${t("shelf.lastRead")} · ${formatReadTime(book.lastReadAt)}`
												]
											})
										]
									}),
									react_jsx_runtime.jsxs("div", {
										className: css.shelfRowActions,
										onClick: (event) => event.stopPropagation(),
										children: [
											react_jsx_runtime.jsx("button", {
												type: "button",
												className: clsx(css.shelfPinBtn, book.pinned && css.shelfPinBtnActive),
												title: book.pinned ? t("shelf.unpin") : t("shelf.pin"),
												"aria-label": book.pinned ? t("shelf.unpin") : t("shelf.pin"),
												onClick: () => onPin(book),
												children: book.pinned ? "★" : "☆"
											}),
											react_jsx_runtime.jsx("button", {
												type: "button",
												className: css.shelfDelBtn,
		...
		function ExplorerAction({ wide, t, workspacesStore, sessionsStore, insertFileReference, openPath, settingsScope }) {
			...
			// Shelf state: which pane the left column shows (file tree vs bookshelf),
			// plus the cached shelf list and its search filter.
			const [activePane, setActivePane] = react.useState("tree");
			const [shelfBooks, setShelfBooks] = react.useState([]);
			const [shelfLoading, setShelfLoading] = react.useState(false);
			const [shelfQuery, setShelfQuery] = react.useState("");
			const refreshShelf = () => {
				setShelfLoading(true);
				apiShelfGet().then((value) => {
					setShelfBooks(value !== null && typeof value === "object" && Array.isArray(value.books) ? value.books : []);
					setShelfLoading(false);
				}, () => { setShelfLoading(false); });
			};
			react.useEffect(() => {
				if (activePane === "shelf") refreshShelf();
			}, [activePane]);
			...
			// A shelf open whose file cannot be read (moved/deleted): warn and ask
			// whether to drop the shelf entry. State (progress/bookmarks) is kept.
			const handleShelfOpenFailure = (name, path, reason) => {
				showNotice(tx("shelf.openFailed"));
				if (typeof window === "undefined") return;
				if (window.confirm(tx("shelf.brokenRemove", { name }))) {
					apiShelfRemove(path, true).then(() => {
						refreshShelf();
						showNotice(tx("shelf.removed"));
					}, (err) => { showNotice(errorMessage(err)); });
				}
			};
			// Open a shelf book: reading mode on, panel open, progress restored by
			// the ReadingView restore effect from the persisted state.
			const openShelfBook = (book) => {
				const name = typeof book.title === "string" && book.title !== "" ? book.title : book.path;
				setOpen(true);
				openPreview(name, book.path, { reading: true, fromShelf: true });
			};
			const handleShelfPin = (book) => {
				const next = book.pinned !== true;
				apiShelfPin(book.path, next).then(() => {
					setShelfBooks((current) => current.map((b) => b.path === book.path ? { ...b, pinned: next } : b));
				}, (err) => { showNotice(errorMessage(err)); });
			};
			const handleShelfRemove = (book) => {
				const name = typeof book.title === "string" && book.title !== "" ? book.title : book.path;
				if (typeof window === "undefined") return;
				// Confirm the removal, then ask whether to also drop the reading
				// progress/bookmarks. The physical file is never touched.
				if (!window.confirm(tx("shelf.removeConfirm", { name }))) return;
				const removeState = window.confirm(tx("shelf.removeAllConfirm", { name }));
				apiShelfRemove(book.path, removeState === true ? false : true).then(() => {
					setShelfBooks((current) => current.filter((b) => b.path !== book.path));
					showNotice(tx("shelf.removed"));
				}, (err) => { showNotice(errorMessage(err)); });
			};
			const openPreview = (name, path, opts) => {
				const fromShelf = opts !== null && opts !== void 0 && opts.fromShelf === true;
				const forceReading = opts !== null && opts !== void 0 && opts.reading === true;
				...
				setReadingMode(forceReading === true);
				if (forceReading !== true) setWritingMode(false);
				...
				apiRead(path, { maxBytes: 4 * 1024 * 1024 }).then((value) => {
					...
					// A shelf open that cannot read the file: offer to drop the
					// entry so a moved/deleted file doesn't stay stuck on the shelf.
					if (fromShelf) handleShelfOpenFailure(name, path, reason);
				});
			};
								...
								children: [
									...
									react_jsx_runtime.jsxs("div", {
										className: css.sidebarActions,
											children: [
												editable && react_jsx_runtime.jsx("button", {
													type: "button",
													className: clsx(css.headerBtn, css.headerBtnPrimary),
													disabled: !edit.dirty || edit.saving,
													onClick: saveEdit,
													children: edit.saving ? tx("files.saving") : tx("files.save")
												}),
												preview !== null && preview.status === "ready" && react_jsx_runtime.jsx("button", {
													type: "button",
												...
												react_jsx_runtime.jsx("button", {
													type: "button",
													className: clsx(css.sidebarClose, activePane === "shelf" && css.shelfToggleActive),
													title: activePane === "shelf" ? tx("panel.title") : tx("shelf.entry"),
													"aria-label": activePane === "shelf" ? tx("panel.title") : tx("shelf.entry"),
													"aria-pressed": activePane === "shelf",
													onClick: () => setActivePane((current) => current === "shelf" ? "tree" : "shelf"),
								...
								children: [
									...
									react_jsx_runtime.jsx("div", {
										...
										react_jsx_runtime.jsx("div", {
											...
											onMouseDown: activePane === "tree" ? (event) => {
											...
											onContextMenu: activePane === "tree" ? (event) => {
												event.preventDefault();
												event.stopPropagation();
												setRootCtxMenu({ x: event.clientX, y: event.clientY });
											} : void 0,
											children: activePane === "tree" ? visibleItems.map((workspace) => react_jsx_runtime.jsx(WorkspaceRoot, {
												workspace,
												t: tx,
											...
											}, workspace.workspaceId)) : react_jsx_runtime.jsx(ShelfView, {
												books: shelfBooks,
												loading: shelfLoading,
												query: shelfQuery,
												onChangeQuery: setShelfQuery,
												onOpen: openShelfBook,
												onPin: handleShelfPin,
